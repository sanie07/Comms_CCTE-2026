 ...
 *  - Bit-stuffing HDLC
 *  - Codificación NRZI (para 1200 bps AFSK/Bell 202)
 ...
 *
 * Equivalente a AX25Frame de RadioLib, portado a C con buffers estáticos.
 * Úsala para construir cualquier tipo de trama AX.25 (UI, I, S, U).
 */
typedef struct {
 ...
 *
 * Equivalente a AX25Client de RadioLib, portado a C.
 * Inicializar con AX25SG_Init() antes de usar.
 */
typedef struct {
    ...
    uint32_t scramblerPoly;                              /**< Polinomio scrambler (0 = desactivado)       */
 ...
 * @param preamble   Cantidad de bytes 0x7E de preámbulo (recomendado: 8-16).
...
/**
 * @brief Configura el scrambler G3RUH (para 9600 bps).
 *
 * Para activar el scrambler G3RUH usar:
 *   AX25SG_SetScrambler(&client, AX25SG_SCRAMBLER_G3RUH_POLY, AX25SG_SCRAMBLER_G3RUH_INIT);
 * Para desactivar (1200 bps), pasar poly = 0.
 *
 * @param client  Puntero al cliente.
 * @param poly    Polinomio del scrambler.
 ...
 *
 * @param frame        Puntero a la trama a inicializar.
 * @param destCallsign Callsign destino.
 * @param destSSID     SSID destino.
 * @param srcCallsign  Callsign origen.
 * @param srcSSID      SSID origen.
 * @param control      Campo de control (p.ej. AX25SG_CTRL_UFRAME | AX25SG_CTRL_U_UI).
 */
 ...
 *
 * Atajo conveniente para el caso más habitual en APRS.
 *
 * @param frame        Puntero a la trama.
 * @param destCallsign Callsign destino.
 * @param destSSID     SSID destino.
 * @param srcCallsign  Callsign origen.
 * @param srcSSID      SSID origen.
 * @param info         Cadena de texto nula-terminada del campo Info.
 * @return  0 si OK, -1 si parámetros inválidos.
 */
 ...
 *
 * @param frame     Puntero a la trama.
 * @param callsigns Array de callsigns de repetidores.
 * @param ssids     Array de SSIDs de repetidores.
 * @param count     Cantidad de repetidores (máx. AX25SG_MAX_REPEATERS).
 * @return  0 si OK, -1 si parámetros inválidos.
 */
...
/**
 * @brief Atajo para transmitir un string de texto como trama UI.
 *
 * Equivalente a AX25Client::transmit() de RadioLib.
 * Serializa internamente y llama a la función de callback de transmisión.
 *
